...
#define CONN_SUP_TIMEOUT                MSEC_TO_UNITS(40000, UNIT_10_MS)        /**< Connection supervisory time-out (4 seconds). */
...
#define NEXT_CONN_PARAMS_UPDATE_DELAY   APP_TIMER_TICKS(50000)                   /**< Time between each call to sd_ble_gap_conn_param_update after the first call (5 seconds). */
#define MAX_CONN_PARAMS_UPDATE_COUNT    3                                       /**< Number of attempts before giving up the connection parameter negotiation. */

#define DEAD_BEEF                       0xDEADBEEF                              /**< Value used as error code on stack dump, can be used to identify stack location on stack unwind. */

#define TX_INTERVAL                     APP_TIMER_TICKS(100)
APP_TIMER_DEF(m_tx_timer_id);

static gatt_srv_t m_srv;
NRF_SDH_BLE_OBSERVER(m_srv_obs, 2, gatt_srv_on_ble_evt, &m_srv);
...
static void advertising_init(void)
{
    ...
    ble_uuid_t adv_uuids[] = {{SRV_UUID_SERVICE, m_srv.uuid_type}};
...
/* Function for handling write events to the RX characteristic. */
static void srv_write_handler(uint16_t conn_handle, gatt_srv_t * p_srv, const uint8_t *data, int len)
{
...
static void services_init(void)
{
    ...
    init.srv_write_handler = srv_write_handler;

    err_code = gatt_srv_init(&m_srv, &init);
    APP_ERROR_CHECK(err_code);
}
